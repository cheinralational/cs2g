...
class CrossGEODDIMSamplerSphere(CVUSA_DDIMSampler):
    """DDIM sampler with spherical control support for crossgeo (256x256 satellite -> 256x256 ground)."""

    def _compute_control_features(self, x, t, c):
        """Compute spherical control features from satellite image."""
        if not hasattr(self.model, 'spherical_controlnet') or self.model.spherical_controlnet is None:
            return None
        sat = getattr(self, '_sat_img', None)
        if sat is None:
            return None
        from torch.nn import functional as F_interp
        hint = F_interp.interpolate(sat, size=x.shape[2:], mode='bilinear', align_corners=False)
        return self.model.spherical_controlnet(x=x, hint=hint, timesteps=t, context=c)
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            control_grd = self._compute_control_features(x, t, c)
            e_t = self.model.denoise_model(x, t, context=c, control_grd=control_grd)
        else:
            x_in = torch.cat([x] * 2)
            t_in = torch.cat([t] * 2)
            c_in = torch.cat([unconditional_conditioning, c])
            ...
            if control_grd is not None:
                control_grd_in = torch.cat([torch.zeros_like(control_grd), control_grd])
            else:
                control_grd_in = None
            e_t_uncond, e_t = self.model.denoise_model(x_in, t_in, context=c_in, control_grd=control_grd_in).chunk(2)
            e_t = e_t_uncond + unconditional_guidance_scale * (e_t - e_t_uncond)
