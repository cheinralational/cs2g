...
class VIGOR_DDIMSampler(object):
    ...
    def triplet_loss(self, x, orin_sat_feat, meter_per_pixel):
        B = x.size()[0]
        ...
        optical = VIGOR_grd2sat_uv(0, x.size(2), x.size(3), 2, A, A, meter_per_pixel).to(x.device)
        ...
        sat_noisy_patch_middle = self.sat_solver(sat_patch.reshape(B*49, 4, crop_H, crop_W)).reshape(B, 49, 4*crop_W*crop_H)
    ...
    #x:the result of xt，c:condition，t
    def cond_fn(self, x, c, text_embed, orin_sat_feat, inter_ref, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        para_dict = {
            "l_clip_global": 0,
            "l_clip_global_patch": 15000, #The coefficient can be adjusted.
            "l_triplet": 0,
                }
        clip_loss = 0    
        clip_patch_loss = 0
        ...
        with torch.set_grad_enabled(True):
            ...
            loss = torch.tensor(0)
            if para_dict["l_clip_global"] != 0: #all image and txt loss
                clip_loss = self.clip_global_loss(x_in_decoder, text_embed) * para_dict["l_clip_global"]
                loss = loss + clip_loss
            
            if para_dict["l_clip_global_patch"] != 0: #echa patch and txt loss
    ...
    def cond_sat_fn(self, x, c, text_embed, orin_sat_feat, inter_ref, t, index, repeat_noise=False, 
                ...
                temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
                unconditional_guidance_scale=1., unconditional_conditioning=None, meter_per_pixel=None):
        ...
        with torch.set_grad_enabled(True):
            ...
            l_triplet = self.triplet_loss(x_in, orin_sat_feat_t, meter_per_pixel)
            grd = torch.autograd.grad(l_triplet, H_matrix)[0]
            grd = grd * 0.5 * 0.9 ** (50 - index) # The coefficient can be adjusted.
    ...
    def ddim_sampling(self, cond, txt_embed, orin_sat_feat, inter_ref, shape,
        ...
        for i, step in enumerate(iterator):
            ...
            outs = self.condition_score(img, cond, txt_embed, orin_sat_feat, inter_ref, ts, index=index,       
                                      ...
                                      corrector_kwargs=corrector_kwargs,
                                      unconditional_guidance_scale=unconditional_guidance_scale,
                                      unconditional_conditioning=unconditional_conditioning, meter_per_pixel=meter_per_pixel)
            img, pred_x0 = outs
