...
def infoNCELoss(scores, labels, temperature=0.1):
    ...
    bool_mask = labels>2e-1 # elements with a likelihood > 1e-2 are considered as positive samples in contrastive learning    
...
class grd_solve(nn.Module):
    def __init__(self, pth_path=None, encoder_model=None, quant_conv=None):
        ...
        self.head = nn.Sequential(
            nn.Linear(16*26*26, 16*26*26),
            nn.SELU(),
            nn.Linear(16*26*26, 4*26*26),
        )

        if pth_path is not None:
            self.load_state_dict(torch.load(pth_path, weights_only=False), strict=True)
