...
class KITTI_DDIMSampler(object):
    def __init__(self, model, pre_AE_model, scale_factor, grd_solve_pth=None, sat_solve_pth=None, schedule="linear", **kwargs):
        super().__init__()
        self.model = model
    ...
    def _ddim_noise(self, index, x):
        if not hasattr(self, '_noise_cache'):
            self._noise_cache = {}
        if index not in self._noise_cache:
            self._noise_cache[index] = torch.randn(x.shape[1:], device='cpu')
        return self._noise_cache[index].unsqueeze(0).to(x.device)
    ...
    def clip_global_patch_loss(self, x_in, text_embed):
        B = x_in.size()[0]
        clip_loss = torch.tensor(0)
        augmented_input = self.patch_augmentations(x_in,num_patch=16).add(1).div(2)
        clip_in = self.clip_normalize(augmented_input)
        image_embeds = self.clip_model.encode_image(clip_in).float()
        dists = self.d_clip_loss(image_embeds, text_embed)
        for i in range(B):
    ...
    def triplet_loss(self, x, orin_sat_feat, left_camera_k):
        B = x.size()[0]
        ...
        BEV_map,_,_,mask = project_grd_to_map(x, None, left_camera_k, 32, 128, 512)
        ...
        for h in range(0, orin_sat_feat.shape[-2] - crop_H + 1, 1):
            for w in range(0, orin_sat_feat.shape[-1] - crop_W + 1, 1):
                orin_sat_feat_t = orin_sat_feat[:,:,h:h+crop_H,w:w+crop_W] * mask
    ...
    def cond_fn(self, x, c, text_embed, orin_sat_feat, inter_ref, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        para_dict = {
            "l_clip_global": 0,
            "l_clip_global_patch": 20000, #5000 15000
            "l_triplet": 0,
            }
        clip_loss = 0    
        clip_patch_loss = 0
        ...
        with torch.set_grad_enabled(True):
            ...
            # if para_dict["l_triplet"] != 0:
            #     orin_sat_feat_t = self.model.q_sample(orin_sat_feat, t)
            #     orin_sat_feat_t = orin_sat_feat * fac + orin_sat_feat_t * (1 - fac)

            #     grid = ORIN_GRID.repeat(x_in.size()[0],1,1,1)
            #     inter_ref = F.grid_sample(x_in, grid)
            #     l_triplet = self.triplet_loss(x_in, orin_sat_feat, inter_ref) * para_dict["l_triplet"]
            #     loss = loss + l_triplet

            # print("loss:",loss,"  clip_loss",clip_loss,"  clip_patch_loss",clip_patch_loss, "l_triplet", l_triplet)
    ...
    def cond_sat_fn(self, x, c, text_embed, orin_sat_feat, inter_ref, t, index, repeat_noise=False, 
        ...
        with torch.set_grad_enabled(True):
            ...
            grid = F.affine_grid(H_matrix[:,:2,:], x.shape)
            ...
            l_triplet = self.triplet_loss(x_in, orin_sat_feat_t, left_camera_k)
            grd = torch.autograd.grad(l_triplet, H_matrix)[0]
            grd = grd * 0.1 * 0.8 ** (50 - index)
    ...
    def cond_sat_test_fn(self, x, c, text_embed, orin_sat_feat, inter_ref, t, index, repeat_noise=False, 
        ...
        l_triplet = self.triplet_loss(x_in, orin_sat_feat_t, inter_ref, left_camera_k)
        print("=========================")
        print(l_triplet)
        return x
    ...
    def p_sample_ddim(self, x, c, t, index, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            ...
            e_t_uncond, e_t = self.model.denoise_model(x_in, t_in, context=c_in, control_grd=None).chunk(2)
        ...
        noise = sigma_t * self._ddim_noise(index, x) * temperature
        if noise_dropout > 0.:
