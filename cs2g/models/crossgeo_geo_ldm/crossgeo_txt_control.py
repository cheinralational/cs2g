...
from models.crossgeo_geo_ldm_diffusion.crossgeo_ddim import CVUSA_DDIMSampler, CrossGEODDIMSamplerSphere
import random
...
class CrossGEO_Sat2Den_ddpm(pl.LightningModule):
    def __init__(self,
                # Sat2Den_config,
                # Sat2Den_ckpt_path,
                AE_config,
                ...
                # control_txt,
                pre_ldm_model_path,
                use_spherical_control=False,
                SphericalControlNet_config=None,
        ...
        if pre_ldm_model_path is not None:
            pre_ldm_model = torch.load(pre_ldm_model_path, weights_only=False)
        ...
        if pre_sat2grd_model_path is not None:
            pre_sat2grd_model = torch.load(pre_sat2grd_model_path, weights_only=False)
            self.load_pre_sat2grd_model(pre_sat2grd_model['state_dict'])

        self.use_spherical_control = use_spherical_control
        self.RMSE = []
        self.SSIM = []
        self.PSNR = []
        self.SD = []
    ...
    def init_AE(self, AE_config, AE_ckpt_path):
        ...
        checkpoint = torch.load(AE_ckpt_path, weights_only=False)['state_dict']
    ...
    def init_Sat2Den(self, Sat2Den_config, AE_ckpt_path):
        ...
        for param in model.parameters():
            param.requires_grad = False
        checkpoint = torch.load(AE_ckpt_path, weights_only=False)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.use_spherical_control and self.DDPM.spherical_controlnet is not None:
            t = torch.randint(0, self.DDPM.num_timesteps, (pre_residual_laten.shape[0],),
                              device=pre_residual_laten.device).long()
            loss = self.DDPM.p_losses_sphere(pre_residual_laten, t, cond_txt=cond_label,
                                              spherical_controlnet_cond=inputs)
        else:
    ...
    def test_step(self, batch, batch_idx):
        ...
        pre_residual_laten = self.pre_AE_model.encode(outputs).sample()
        _, c, h, w = pre_residual_laten.shape
        shape_single = [1, c, h, w]
        x_T = torch.randn(shape_single, device=inputs.device).repeat(sat_con.size()[0], 1, 1, 1)
        sampler = CrossGEODDIMSamplerSphere(self.DDPM, self.pre_AE_model, self.scale_factor)
        sampler._sat_img = inputs
        ...
        #     name = os.path.basename(grd_path[batch_num]).split('.')[0]
        #     val_save = toPIL(pre_residual[batch_num])
        #     val_save.save(f"result/2024-09-09T12-37-15_geo_ldm_txt_control/test/test_img/{name}.jpg")
    ...
    def configure_optimizers(self):
        ...
        params = list(self.DDPM.denoise_model.parameters()) + list(self.condition_model_sat.parameters())
        if self.use_spherical_control and self.DDPM.spherical_controlnet is not None:
            params += list(self.DDPM.spherical_controlnet.parameters())
