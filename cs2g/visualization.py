...
if __name__ == "__main__":
    ...
    state_dict = torch.load(opt.test, weights_only=False)['state_dict']
    ...
    with torch.no_grad():
        for batch in test_dataset:
            if 'CVUSA' in config.data.params.test.target:
                ...
                dummy = torch.zeros(1, 3, 256, 256).cuda()
                dummy = dummy * 2 - 1
                enc = model.pre_AE_model.encode(dummy).sample()
                _, c, h, w = enc.shape
                shape_full = [1, c, h, w]
                x_T = torch.randn(shape_full, device=inputs.device).repeat(sat_con.size()[0], 1, 1, 1)
                start_code = x_T
