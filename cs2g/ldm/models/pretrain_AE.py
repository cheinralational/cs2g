...
from geometry.sat2grd import sat2grd_uv, grd2sat_uv, grid_sample
...
class AutoencoderKL(pl.LightningModule):
    def __init__(self,
        ...
        #冻结decoder
        self.decoder = Decoder(**ddconfig)
        test_checkpoint = torch.load('logs/2023-12-07T00-28-24_autoencoder_kl_16x16x16/checkpoints/epoch_59.ckpt', weights_only=False)
        
        new_state_dict = OrderedDict()
        for k, v in test_checkpoint['state_dict'].items():
            if 'decoder' in k:
    ...
    def encode(self, x):
        h = self.encoder(x)#下采样 提取特征 torch.Size([70, 32, 16, 16])
        moments = self.quant_conv(h) #torch.Size([70, 32, 16, 16])
    ...
    def forward(self, input, sample_posterior=True):
        ...
        if sample_posterior:
            z = posterior.sample() #VAE
        else:
            z = posterior.mode()
        #转化为地面全景图
        # meter_per_pixel = torch.ones(z.size()[0], dtype=torch.float32, device=z.device) * 0.21
        # optical = grd2sat_uv(level = 2, grd_H = 128, grd_W = 512, grd_cam_h = 2, 
        #                      sat_H = z.size()[2], sat_W = z.size()[3], meter_per_pixel = meter_per_pixel)
        # z, mask = grid_sample(z, optical)
        z = z.reshape(z.size()[0], -1, 8, 32)
        # img = transforms.ToPILImage()(z[0,:3])
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        # inputs = self.get_input(batch, self.outputs)
        outputs = self.get_input(batch, self.outputs)
    ...
    def test_step(self, batch, batch_idx):
        ...
        # inputs = self.get_input(batch, self.outputs)
        outputs = self.get_input(batch, self.outputs)
        reconstructions, posterior = self(inputs, True)
